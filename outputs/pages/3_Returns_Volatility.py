...
with tab_ret:
    ...
    if bench_m:
        ...
        st.altair_chart(va + vl + bl, use_container_width=True)
        st.caption(f"Blue = {ticker}  ·  Orange = {benchmark}")
    else:
        st.altair_chart(va + vl, use_container_width=True)
...
with tab_dd:
    ...
    st.altair_chart(dda + ddl, use_container_width=True)
...
with tab_dist:
    ...
    st.altair_chart(hist + nl + mr, use_container_width=True)
...
with tab_month:
    ...
    if not mt.empty:
        ...
        st.dataframe(mt.style.background_gradient(cmap="RdYlGn", axis=None, vmin=-10, vmax=10).format("{:.1f}%", na_rep="—"), use_container_width=True)
    else:
