...
with col_note:
    ...
    st.dataframe(breakdown_df.set_index("Dimension"), use_container_width=True)
...
with tab_risk:
    ...
    st.altair_chart(dd_chart + dd_line, use_container_width=True)
...
with tab_returns:
    ...
    if bench_m:
        ...
        st.altair_chart(vol_area + vol_line + bl, use_container_width=True)
        st.caption(f"Blue = {ticker}  ·  Orange dashed = {benchmark}")
    else:
        st.altair_chart(vol_area + vol_line, use_container_width=True)
